...
export function DatosUser({openDesplegableUser, setOpenDesplegableUser}) {
  const navigate = useNavigate();
  const { user } = UserAuth();
  const { signout,datauserGoogle } = useAuthStore();
  ...
  return (
    <Container onClick={() => setOpenDesplegableUser(!openDesplegableUser)}>
      <div className="imgContainer">
        <img src={datauserGoogle.picture} />
      </div>
      ...
      <span className="nombre">{datauserGoogle.name}</span>
      {openDesplegableUser && (
        <ContentMenuDesplegable
          top="60px"
          datadesplegable={DesplegableUser}
